# pelicana_crawler.py : 페리카나 국내 매장 크롤링
...
def getPelicanaAddress(result) :
    for page_idx in count() :
        # print(page_idx)
        # time.sleep(1)
        url = 'https://pelicana.co.kr/store/stroe_search.html?&branch_name=&gu=&si=&page=%s' % str(page_idx + 1)
        ...
        # print(tbody)
        bEnd = True
        for store_tr in tbody.findAll('tr') :  # findAll -> 자동으로 리스트
            bEnd = False
            ...
            result.append([store_name] + store_sido_gu + [store_address])  # store_sido_gu : 리스트 타입이라 그냥 넣어도 됨
        if (bEnd == True) :
            print(result[0])
