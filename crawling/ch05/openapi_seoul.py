...
# [CODE 2] '서울 열린데이터 광장' : 한 page에 5개만 display 됨
def getPage(start):
...
def main() :
    ...
    print("<<현재 기준 '민주주의 서울 자유 제안' 데이터 100개를 수집합니다.>>")
    result = getItemsAll()

    # 파일 저장
    columns = ["SN", "TITLE", "CONTENT_link", "DATE"]
    result_df = pd.DataFrame(result, columns=columns)
    result_df.to_csv('./data/민주주의서울자유제안.csv', index=False, encoding='cp949')  # cp949 : code page 949
